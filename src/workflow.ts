...
// Use GitHub API to trigger a workflow instead of CircleCI
const triggerGitHubWorkflow = async (
  ...
  try {
    ...
    const response = await args.octokit.rest.actions.createWorkflowDispatch({
    ...
    // Try to get the latest workflow run to return its URL
    const runs = await args.octokit.rest.actions.listWorkflowRuns({
      owner: github.context.repo.owner,
      repo: github.context.repo.repo,
      workflow_id: workflowId,
      per_page: 1
    ...
    const workflowRunId = runs.data.workflow_runs[0]?.id
    ...
    core.error(`Error triggering GitHub workflow: ${error}`)
    throw error
  }
}
...
export const workflow = {
  ...
  async createTicket(
    ...
    // Convert ticketParams to GitHub Actions inputs format
    const workflowInputs = Object.entries(ticketParams).reduce(
      (acc, [key, value]) => {
    ...
    // Define your GitHub workflow ID - could be file name like "create-ticket.yml"
    const workflowId = 'create-ticket.yml'

    const {repository, workflowRunId, response} = await triggerGitHubWorkflow(
      commandArgs,
      workflowId,
      workflowInputs
    )
    ...
    // Build workflow URL for GitHub Actions
    const workflowUrl = workflowRunId
      ? `https://github.com/${repository}/actions/runs/${workflowRunId}`
      : `https://github.com/${repository}/actions/workflows/${workflowId}`
