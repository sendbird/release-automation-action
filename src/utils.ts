...
export function isReleaseBranch(branch: string): boolean {
  return [BRANCH_RELEASE_PREFIX, BRANCH_HOTFIX_PREFIX].some((prefix) => branch.startsWith(prefix));
}

export function extractVersion(branch: string): string {
  const versionRegex = getVersionRegex([BRANCH_RELEASE_PREFIX, BRANCH_HOTFIX_PREFIX]);
...
export function buildJiraVersionPrefix(platform: string, product: string, framework = ''): string {
...
export function buildReleaseJiraVersion(platform: string, product: string, version: string, framework = ''): string {
...
export function buildReleaseJiraTicket(platform: string, product: string, version: string, framework = ''): string {
  // [{product}]{platform}[-{framework}]?@{version}
  // [Live-UIKit] js-react@0.0.0, [Chat] ios@0.0.0
