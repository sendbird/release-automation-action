...
import type { IssueCommentCreatedEvent, WebhookEventName } from '@octokit/webhooks-definitions/schema';
...
async function run(): Promise<void> {
  try {
    if ((github.context.eventName as WebhookEventName) === 'issue_comment') {
      ...
      if (command) {
        const hasPermission = await checkPermission(payload.comment.user.login, octokit);
...
async function checkPermission(username: string, octokit: ReturnType<typeof github.getOctokit>): Promise<boolean> {
  if (username === github.context.repo.owner || username === SENDBIRD_BOT_USERNAME) {
    return true;
  }
